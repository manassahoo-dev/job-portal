...
function Aptitude(params) {
    ...
    return (
        <Card bordered={false} title={<Text strong>Aptitude Tests</Text>}>
            <Tabs defaultValue="Upcoming" type="card" style={contentStyle} onChange={(e) => setSelectedBatchId(e)}>
                {contextData?.batches?.map((batch, index) =>
                    <TabPane tab={batch.name} key={batch.id}>
                        <Text italic>20 Students Aptitude test will conduct today 03 Sept, 21</Text>
